...
#+begin_src emacs-lisp
(when (and (selected-frame) (display-graphic-p)) (x-focus-frame nil))
#+end_src
...
#+begin_src emacs-lisp
(after! evil-vars
  (push! evil-emacs-state-modes 'dired-mode))
#+end_src
...
As of [[github:emacs-evil/evil-collection/commit/e7be9f53a05c81ef9e89c2f053ba62048e6afd69][=e7be9f5=]], ~evil-collection~ throws an error when starting
~helpful-mode~, because it can’t find ~evil-collection-indent~. This workaround
disables the message by disabling the entire ~evil-collection-indent~ package.
#+begin_src emacs-lisp
