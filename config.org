...
(defadvice! *emacs--disable-indent-tabs-mode-a (fn &rest args)
  "Call original function with `indent-tabs-mode' bound to nil."
  :around 'align-areas
...
Sometimes Emacs should just tab to the next tab stop.
#+begin_src emacs-lisp
(map! "S-<tab>"   #'tab-to-tab-stop
...
Reimplement Evil’s block-mode insertion operators using ~string-rectangle~,
because it has a nice preview overlay and can be canceled using =C-g=. This also
fixes the coarse undo problem for these operators.
#+begin_src emacs-lisp
(defun *evil-visual-change ()
  "Reimplement `evil-change' in visual-block-mode using `string-rectangle'."
  (interactive)
  (call-interactively
   (if (and (evil-visual-state-p) (eq evil-visual-selection 'block))
       #'string-rectangle #'evil-change)))

(defun *evil-visual-change-line ()
  "Reimplement `evil-change-line' in visual-block-mode using `string-rectangle'."
  (interactive)
  (if (and (evil-visual-state-p) (memq evil-visual-selection '(line block)))
      (let ((block-beg evil-visual-beginning)
            (block-end (if (eq evil-visual-selection 'line) (1- evil-visual-end) evil-visual-end))
            (point (point))
            (not-vis-mode (not (and evil-respect-visual-line-mode visual-line-mode)))
            (suffices nil)
            (ok nil))
        (evil-apply-on-block
         (lambda (beg end)
           (let ((line-end (if not-vis-mode (point-at-eol)
                             (goto-char beg) (end-of-visual-line) (point))))
             (push (buffer-substring beg line-end) suffices)
             (delete-region beg line-end)))
         block-beg block-end nil)
        (unwind-protect (progn (*evil-visual-insert) (setq ok t))
          (unless ok
            (setq suffices (nreverse suffices))
            (evil-apply-on-block
             (lambda (beg end) (goto-char beg) (insert (pop suffices)))
             block-beg block-end nil)
            (goto-char point))))
    (call-interactively #'evil-change-line)))

(defun *evil-visual-insert ()
  "Reimplement `evil-insert' in visual-block-mode using `string-rectangle'."
  (interactive)
  (cond ((not (evil-visual-state-p)) (call-interactively #'evil-insert))
        ((eq evil-visual-selection 'line)
         ...
         (save-mark-and-excursion
           ...
           (forward-line -1)
           (unwind-protect (call-interactively #'string-rectangle)
             (rectangle-mark-mode -1))))
         ...
         (let ((column (min (evil-column evil-visual-beginning)
           ...
           (goto-char
            (save-mark-and-excursion
              ...
              (set-mark (save-excursion (goto-char (mark)) (move-to-column column t) (point)))
              (call-interactively #'string-rectangle)
              (point)))))
        ((call-interactively #'evil-insert))))

(defun *evil-visual-append ()
  "Reimplement `evil-append' in visual-block-mode using `string-rectangle'."
  (interactive)
  (if (and (evil-visual-state-p) (eq evil-visual-selection 'block))
...
(map! :after evil
      :v "c" #'*evil-visual-change
      :v "C" #'*evil-visual-change-line
      :v "I" #'*evil-visual-insert
      :v "A" #'*evil-visual-append)
#+end_src
...
Change =vslot= of =Org Src= popup, so it is not replaced by =helpful= popups.
#+begin_src emacs-lisp
(when (featurep! :ui popup)
  (advice-add
   #'+org-init-popup-rules-h :after
   (lambda (&rest _)
     (setf (cdr (assq 'vslot (cdr (assoc "^\\*Org Src" +popup--display-buffer-alist)))) -1))))
#+end_src
...
** C/C++
When commenting out part of a line in C/C++, for example the name of a function
argument, this comment function intelligently switches to block comment mode.
Maybe I should generalize this?
#+begin_src emacs-lisp
(defun *c-comment-region (beg end &optional arg)
  "Comment from BEG to END using C-style comments for short regions.
Like `comment-region-default', but uses `c-block-comment-starter' etc
when the region only contains part of a single line."
  (goto-char beg)
  (if (and (<= (line-beginning-position) beg)
           (< end (line-end-position))
           c-block-comment-starter (not c-block-comment-flag))
      (let ((comment-start (concat c-block-comment-starter " "))
            (comment-end   (concat " " c-block-comment-ender)))
        (comment-region-default beg end arg))
      (comment-region-default beg end arg)))

(setq-hook! 'c-mode-common-hook
  comment-region-function #'*c-comment-region)
#+end_src
...
*** Bison Mode
These C++-like extensions for Lex/Yacc files are recognized by the GNU tools as
containing C++ code. While =bison-mode= only supports C, that is still better
than having no support at all.
#+begin_src emacs-lisp
(push! auto-mode-alist
       '("\\.l\\(l\\|pp\\|xx\\|\\+\\+\\)\\'" . flex-mode)
       '("\\.y\\(y\\|pp\\|xx\\|\\+\\+\\)\\'" . bison-mode))
#+end_src
...
(setq-hook! 'lisp-mode-hook
  comment-multi-line t ;; Fix block comments.
  fill-column 100)
#+end_src
...
Many Common Lisp projects follow the convention to have a single =packages.lisp=
file in the project root containing the definition of the project’s packages.
#+begin_src emacs-lisp
(after! projectile
  (push "packages.lisp" projectile-project-root-files))
#+end_src
...
*** Highlight Quoted
Redefine the regexes used by =highlight-quoted= to not trigger on escaped
quotation characters.
#+begin_src emacs-lisp
(after! highlight-quoted
  (defconst highlight-quoted--quote-only-keywords
    `((,(rx (or bol (not (syntax escape))) (group (or "`" "'" "#'")))
       (1 'highlight-quoted-quote))))
  (defconst highlight-quoted--full-keywords
    `((,(rx (or bol (not (syntax escape)))
            (group (or "`" "'" "#'"))
            (? (* whitespace) (group (+ (or (syntax word) (syntax symbol))))))
       (1 'highlight-quoted-quote)
...
MAYBE: Define =python-flit= as a project type for =projectile=. Might not be
needed when [[github:pypa/build][=python-build=]] / [[github:pypa/installer][=python-installer=]] become the standard.

#+begin_src emacs-lisp
(after! flycheck
  (pushnew! flycheck-python-mypy-config "pyproject.toml"))
#+end_src
