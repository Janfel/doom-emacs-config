...
(defvar source-directory-base
  (file-name-as-directory (file-name-nondirectory (directory-file-name source-directory)))
...
#+BEGIN_SRC emacs-lisp
(defvar current-source-directory
  (let ((dir (expand-file-name source-directory-base "~/opt/src/")))
    (if (file-accessible-directory-p dir) dir))
  "Directory where Emacs sources can be found.

This differs from `source-directory' in that it either currently contains
the Emacs source code, or is set to nil.")
#+END_SRC

#+BEGIN_SRC emacs-lisp
(setq find-function-C-source-directory
      (when current-source-directory
       (let ((dir (expand-file-name "src/" current-source-directory)))
        (if (file-accessible-directory-p dir) dir))))
#+END_SRC
