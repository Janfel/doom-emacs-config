...
#+BEGIN_SRC emacs-lisp
(when (featurep! :editor fmt)
  (defvar prettier-fmt-indent standard-indent
  ...
  (reformatter-define prettier-fmt
    ...
    :mode nil
    :args
    `("--tab-width" ,(number-to-string prettier-fmt-indent)
...
(when (featurep! :editor fmt)
  (setq-hook! 'js-mode-hook
    fmt/formatter '(prettier-fmt-buffer . prettier-fmt-region)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(defvar ptop-fmt-config
  (expand-file-name "pascal/ptop.cfg" XDG-CONFIG-HOME)
  "The configuration file for `ptop-fmt'.")
#+END_SRC

Define =ptop-fmt= using =:editor fmt=.
#+BEGIN_SRC emacs-lisp
(when (featurep! :editor fmt)
  (require 'reformatter)

  (defvar ptop-fmt-indent DEFAULT-PASCAL-INDENT
    "Indent level for `ptop-fmt'.")

  (reformatter-define ptop-fmt
    :program "ptop"
    :mode nil
    :args
    `(,@(-some->> ptop-fmt-config (list "-c"))
      "-i" ,(number-to-string (or ptop-fmt-indent standard-indent))
      "/dev/stdin" "/dev/stdout"))

  (setq-hook! '(pascal-mode-hook
                opascal-mode-hook)
    fmt/formatter '(ptop-fmt-buffer . ptop-fmt-region)))
#+END_SRC
...
*** Perl::Tidy
Define =perltidy-fmt= using =:editor fmt=.
#+BEGIN_SRC emacs-lisp
(when (featurep! :editor fmt)
  (require 'reformatter)

  (defvar perltidy-fmt-indent DEFAULT-PERL-INDENT
    "Indent level for `perltidy-fmt'.")

  (reformatter-define perltidy-fmt
    :program "perltidy"
    :mode nil
    :args
    `("--indent-columns" ,(number-to-string perltidy-fmt-indent)
  ...
  (setq-hook! '(perl-mode-hook
                cperl-mode-hook)
    fmt/formatter '(perltidy-fmt-buffer . perltidy-fmt-region)))
#+END_SRC
...
(when (featurep! :editor fmt)
  (setq-hook! 'php-mode-hook
    fmt/formatter '(prettier-fmt-buffer . prettier-fmt-region)
    prettier-fmt-indent c-basic-offset))
#+END_SRC
...
(when (featurep! :editor fmt)
  ...
  (setq-hook! 'python-mode-hook
    fmt/formatter '(black-fmt-buffer . black-fmt-region)))
#+END_SRC
