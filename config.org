...
#+BEGIN_SRC emacs-lisp :tangle no :noweb-ref :noweb no-export
(when EMACS27+
  (after! tab-bar
    <<tab-bar-config>>))
#+END_SRC
...
Disabled due to lack of necessity at the moment.
#+BEGIN_SRC emacs-lisp :tangle no
(when (featurep! :config literate)
  (defadvice! *literate--async-compilation-a (&rest _)
    ...
    (when (and (eq major-mode 'org-mode)
      ...
      (start-process "proc" nil
                     (expand-file-name "org-tangle" doom-bin-dir)
...
*** Sly
:PROPERTIES:
:header-args+: :tangle no :noweb-ref lisp-sly-config
:END:
#+BEGIN_SRC emacs-lisp
