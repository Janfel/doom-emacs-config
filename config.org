...
These constants describe the features each of my systems has.
#+BEGIN_SRC emacs-lisp
(defconst SYSTEM
...
(defconst BLACK-FORMATTER-IS-PATCHED
  (eq SYSTEM 'Phantom)
  "Does this system have a patched Black?")
#+END_SRC
...
*** Black
#+BEGIN_SRC emacs-lisp
(when (and (not BLACK-FORMATTER-IS-PATCHED) (featurep! :editor fmt))
...
#+BEGIN_SRC emacs-lisp
(when (and BLACK-FORMATTER-IS-PATCHED (featurep! :editor fmt))
  (use-package! format-all :commands (format-all--buffer-extension-p))
...
*** PTOP
The variable =ptop-formatter-config= describes where the =ptop= configuration
file is stored. This is important, because I have multiple configurations for
multiple styles.
#+BEGIN_SRC emacs-lisp
(when! (featurep! :editor fmt)
